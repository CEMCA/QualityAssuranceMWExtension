...
"use strict";
/*global $:false */
/* global wgArticleId,wgScriptPath, alert */
/*jshint -W043 */

$( document )
	.ready( function() {

		var $articleId, qaAssesments, userQA, evalDetails, numTemp, key, answersTemp, text;
		...
		$.get( wgScriptPath + "/api.php?action=qaAssesments&qaPageNo=" + $articleId + "&qatype=basic&format=json", function( data ) {
			...
			$( "#roverall" )
				.html( qaAssesments.overallScore + "/4.0" );
			$( "#rT" )
				.html( qaAssesments.TScore + "/4.0" );
			$( "#rI" )
				.html( qaAssesments.IScore + "/4.0" );
			$( "#rP" )
				.html( qaAssesments.PScore + "/4.0" );
			$( "#rS" )
				.html( qaAssesments.SScore + "/4.0" );
			$( "#numResponses" )
				.html( qaAssesments.numResponses + "" );

			if ( qaAssesments.numResponses === 0 ) {
				$( "#ratingInfo" )
					.html( "No quality assessments yet.<br>" );
				$( "#showDetailedInfo" )
					.hide();
			}
		} );
		...
		$( "#showDetailedInfo" )
			.click( function() {
				...
				$( "#assesmentForm" )
					.append( "<br> <h2>Below is the list of QA assessments. </h2><br>" );
				...
				$.get( wgScriptPath + "/api.php?action=qaAssesments&qaPageNo=" + $articleId + "&qatype=detailed&format=json", function( data ) {
					...
					var $table = "<br><table border='1' class='prettytable' width='100%'>";
					$table += "\
				<tr>\
					<th> User Name </th>\
					<th> QA Score </th>\
			...
			";
					...
					for ( var i in data.qaAssesments ) {
					...
					$( "#assesmentForm" )
						.append( $table );
					$( "#assesmentForm" )
						.append( "<span id='detailsHolder'></span>" );
					...
					$( ".details" )
						.click( function() {
							...
							text = "\
					<div id='dialog' title='QA evaluation answers for user " + $( this )
								.attr( "username" ) + "'> \
					  " + evalDetails + "\
					</div>	\
				";
			...
			.click( function() {
				...
				$.get( wgScriptPath + "/api.php?action=query&meta=userinfo&format=json", function( data ) {
					...
					$( "#assesmentForm" )
						.append( "<br>" );
					$( "#assesmentForm" )
						.append( "<br> <h2> Please answer the following questions in order to make the quality assessment. </h2>" );

					var qnum = 1;
					for ( var i in $questions ) {
						var $table = "<span id='qaform" + i + "'>";
					...
					$( "#submitForm0" )
						.click( function() {
							...
							for ( var i = 1; i <= 8; i++ ) {
								$( "#error" + i )
									.html( "" );
								var ans = $( "input[name=" + i + "]:checked" )
									.val();
								if ( !ans ) {
									allAnswered = false;
									$( "#error" + i )
										.html( "<br>Please answer this question." );
								}
								answers[ i ] = ans;
							}
							//console.log( answers );
							if ( !allAnswered ) {
								$( "#errorSubmit0" )
									.html( "<br>You need to answer all questions before proceeding</br>" );
								return false;
							}
							$( "#qaform0" )
								.hide();
